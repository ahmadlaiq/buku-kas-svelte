...
export const load: PageServerLoad = async ({ url, locals }) => {
  if (!locals.user) return { karyawan: [], pagination: { page: 1, limit: 25, totalItems: 0, totalPages: 0 } };
...
export const actions: Actions = {
  ...
  delete: async ({ request, locals }) => {
    ...
    try {
      await prisma.karyawan.delete({ where: { id, ...(locals.user.tenant_id ? { tenant_id: locals.user.tenant_id } : {}) } });
      return { success: true };
    } catch (error) {
