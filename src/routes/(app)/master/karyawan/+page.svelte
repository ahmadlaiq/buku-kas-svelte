...
{/if}
{#if form?.success}
  <div class="alert alert-success" style="margin-bottom: 1rem;">Berhasil menyimpan data karyawan!</div>
{/if}
                ...
                <form method="POST" action="?/delete" use:enhance={() => {
                  ...
                  <button
                    ...
                    on:click|preventDefault={(e) => {
                      if (confirm('Yakin ingin menghapus karyawan ini?')) e.currentTarget.form?.requestSubmit();
                    }}
                  >Hapus</button>
                </form>
              </div>
    ...
    <div class="modal-content">
      ...
      <form method="POST" action="?/create" use:enhance={() => {
        ...
        <div class="form-group">
          ...
          <input type="text" id="nama" name="nama" class="form-control" required placeholder="Masukkan nama karyawan" />
        </div>
        <div class="form-group">
          <label for="posisi" class="form-label">Posisi</label>
          <input type="text" id="posisi" name="posisi" class="form-control" placeholder="Contoh: Terapis, Kasir" />
        </div>
        <div class="form-group">
        ...
        <div style="display: flex; gap: 1rem; margin-top: 1.5rem;">
          ...
          <button type="submit" class="btn btn-primary" style="flex: 1;" disabled={loading}>{loading ? 'Menyimpan...' : 'Simpan Karyawan'}</button>
        </div>
      </form>
    </div>
  </div>
  ...
  <div class="modal">
    <div class="modal-content">
      ...
      <form method="POST" action="?/update" use:enhance={() => {
        ...
        <div class="form-group">
          <label for="edit-nama" class="form-label">Nama Karyawan *</label>
          <input type="text" id="edit-nama" name="nama" class="form-control" required bind:value={editItem.nama} />
        </div>
        <div class="form-group">
          <label for="edit-posisi" class="form-label">Posisi</label>
          <input type="text" id="edit-posisi" name="posisi" class="form-control" bind:value={editItem.posisi} />
        </div>
        <div class="form-group">
