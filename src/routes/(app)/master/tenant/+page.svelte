...
<div class="card">
  <div class="table-container">
    <table class="table">
      <thead><tr><th>Nama Tenant</th><th>Alamat</th><th>No HP</th><th>Email</th><th>Medsos</th><th>Status</th><th style="width:140px;">Aksi</th></tr></thead>
      <tbody>
            ...
            <td>
              <div style="display:flex;gap:0.4rem;">
                ...
                <form method="POST" action="?/delete" use:enhance={() => { loading = true; return async ({ update }) => { loading = false; update(); }; }}>
                  <input type="hidden" name="id" value={item.id} />
                  <button type="submit" class="btn btn-sm btn-danger" disabled={loading} on:click|preventDefault={(e) => { if (confirm('Yakin hapus tenant ini?')) e.currentTarget.form?.requestSubmit(); }}>Hapus</button>
                </form>
              </div>
            </td>
          </tr>
