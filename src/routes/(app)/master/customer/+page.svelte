...
<svelte:head>
  <title>Master Customer - Buku Kas</title>
</svelte:head>

<div class="header">
  <div>
    <h1 class="text-2xl font-bold">Master Customer</h1>
    <p class="text-muted">Kelola data pelanggan salon</p>
  </div>
...
{/if}
{#if form?.success}
  <div class="alert alert-success" style="margin-bottom: 1rem;">Berhasil menyimpan data customer!</div>
{/if}
                ...
                <form method="POST" action="?/delete" use:enhance={() => {
                  ...
                  <button
                    ...
                    on:click|preventDefault={(e) => {
                      if (confirm('Yakin ingin menghapus customer ini?')) e.currentTarget.form?.requestSubmit();
                    }}
                  >Hapus</button>
                </form>
              </div>
  ...
  <div class="modal">
    <div class="modal-content">
      <div class="modal-header">
        <h2 class="text-xl font-bold">Tambah Customer</h2>
      ...
      <form method="POST" action="?/create" use:enhance={() => {
        ...
        <div class="form-group">
          ...
          <input type="text" id="nama" name="nama" class="form-control" required placeholder="Masukkan nama pelanggan" />
        </div>
        <div class="form-group">
          <label for="no_hp" class="form-label">No. HP</label>
          <input type="text" id="no_hp" name="no_hp" class="form-control" placeholder="08123xxxx" />
        </div>
        <div class="form-group">
          <label for="alamat" class="form-label">Alamat</label>
          <textarea id="alamat" name="alamat" class="form-control" placeholder="Contoh: Jl. Merdeka No 1" rows="3"></textarea>
        </div>
        <div style="display: flex; gap: 1rem; margin-top: 1.5rem;">
          ...
          <button type="submit" class="btn btn-primary" style="flex: 1;" disabled={loading}>{loading ? 'Menyimpan...' : 'Simpan Customer'}</button>
        </div>
      </form>
    </div>
  </div>
  ...
  <div class="modal">
    <div class="modal-content">
      ...
      <form method="POST" action="?/update" use:enhance={() => {
        ...
        <div class="form-group">
          <label for="edit-nama" class="form-label">Nama Customer *</label>
        ...
        <div class="form-group">
          ...
          <input type="text" id="edit-no_hp" name="no_hp" class="form-control" bind:value={editItem.no_hp} />
        </div>
        <div class="form-group">
          <label for="edit-alamat" class="form-label">Alamat</label>
          <textarea id="edit-alamat" name="alamat" class="form-control" rows="3" bind:value={editItem.alamat}></textarea>
        </div>
        <div style="display: flex; gap: 1rem; margin-top: 1.5rem;">
